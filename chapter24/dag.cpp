#include<iostream>
#include<cmath>
#include<list>
#include<stack>
#include<bitset>
#include<vector>
#include<string>
#include<algorithm>
#include <new>
#include <map>
#include<utility>
#define INF (1<<30)
#define null NULL
#define fi(sz) for(int i=0;i<sz;i++)
#define fj(sz) for(int j=0;j<sz;j++) 
#define pb push_back
#define all(v) (v).begin(),(v).end()
#define SP system("pause");
#define OP cout<<endl
#define WHITE 0
#define GRAY 1
#define BLACK 2
typedef long long LL;
using namespace std;

struct node{
       int index;
       node* pd; //predecesor
       list<pair<node* ,int> > child; // child
       int ds;  //distance
       int dtime;//discovery time
       int ftime;//finishing time
       int color;
       };

class graph_alg{
      vector<node* > v;int time;

      public:
      graph_alg(int n){v.resize(n,null);time=0;}
      
node* getelement(int i){return v[i];}

void relax(int index_v,int cost,node* u){

   if(v[index_v]->ds>u->ds+cost){
                                 v[index_v]->ds=u->ds+cost;v[index_v]->pd=v[u->index];}

}//relax finish


        
void DFS_visit_TSort(node* x,list<node* > &ll){
     
     time+=1;
     x->dtime=time;
     x->color=GRAY;
     cout<<" node visiting "<<x->index<<"  start time "<<x->dtime;OP;
     for(list<pair<node* ,int>  > :: iterator it=x->child.begin();it!=x->child.end();it++){
                    if((*it).first->color==WHITE){(*it).first->pd=x;DFS_visit_TSort((*it).first,ll);}
                    }
          x->color=BLACK;
          x->ftime=time=time+1;   
          ll.push_front(x);       
     }//dfs visit finish
       
void DFS_TSort(list<node* > &ll){
     for(int i=0;i<v.size();i++){
             if(v[i]->color==WHITE){DFS_visit_TSort(v[i],ll);}
             }
     }    //DFS finish   

list<node* > TSort(){
     list<node* > l;
     DFS_TSort(l);
     return l;
     }//tsort finish

void dag_acyclic_shortest_path(node* source){
     
     list<node* > l=TSort();
     source->ds=0;
     for(list<node* >::iterator it=l.begin();it!=l.end();it++){
                    for(list<pair<node* ,int> > ::iterator it1=(*it)->child.begin();it1!=(*it)->child.end();it1++){
                                relax((*it1).first->index,(*it1).second,(*it));           
                                        }//inner for finish
                    }//outer for finsih
     }//dag shortest finish
    
void make_graph(){
     get_data();
     for(int i=0;i<v.size();i++){

     cout<<"enter no. of edges connected to "<<(i)<<"th vertex and index";OP;
             int temp1;cin>>temp1;
             for(int j=0;j<temp1;j++){
                      int temp2,temp3;cin>>temp2>>temp3;
                      make_link(i,temp2,temp3);
                     }
             }
     }//make arbitrary graph

void make_link(int i,int j,int k){
     v[i]->child.push_back(make_pair(v[j],k));
     }//make link finish

node* make_node(int data){
     node* temp=new node;
     temp->index=data;
     temp->pd=null;
     temp->ds=INF;
     temp->dtime=0;
     temp->ftime=0;
     temp->color=WHITE;
     return temp;
     }//make node finish

void get_data(){
     cout<<"enter "<<v.size()<<" vertices value";OP;
     for(int i=0;i<v.size();i++){
             v[i]=make_node(i);
             }
     }//get data finish


      };


int main(){
    int n; 
    cin>>n;
    graph_alg ga(n);
    ga.make_graph();
    ga.dag_acyclic_shortest_path(ga.getelement(1));
    cout<<" the new ds is";OP;
    for(int i=0;i<n;i++)
    cout<<ga.getelement(i)->ds<<endl;
    OP;
    SP;
    }

/*
6
2 1 5 2 3
2 2 2 3 6
3 3 7 4 4 5 2
2 4 -1 5 1
1 5 -2
0
*/



...
int Get_Median_Select(int m){
     
     int n=A.size();

    for(int i=0;i<=ceil(n/5);i++){int j;
            for(j=5*i;j<5*i+5 && j<n;j++){
                    for(int k=j-1;k>=5*i && A[k]>A[k+1];k--){swap(A[k],A[k+1]);}
                    }
    ...
    // cout<<"y "<<y<<endl;
     if(y==m)   {return A[y];}
     else if(m<y) {return Randomize_Select(0,y-1,m);}
     else{return Randomize_Select(y+1,A.size()-1,m);}
...
int myint;cin>>myint; int i=0;while(i<myint){
   ...
   A.push_back(temp);i++;
    ...
    int first=Get_Median_Select(myint-4);
    vector<int> temp(4,0);
    temp[0]=A[myint-4];
    temp[1]=A[myint-3];
    temp[2]=A[myint-2];
    temp[3]=A[myint-1];
    sort(all(temp));cout<<endl;
    for(int i=3;i>=0;i--)cout<<temp[i]<<endl;
